...
`public/images/company-logo.png` 为公司提供的透明 PNG 标识，仅做网页尺寸优化，没有改变图形结构。页头与页脚以透明通道作为蒙版，使用网站现有深绿色显示；`public/favicon.png` 使用同一标识生成。两项资产均不用于第三方再分发。

## 站点标识

页头、页脚和 favicon 已统一使用公司确认的品牌标识，公司中英文名称仍使用网页文字排版，以保持小尺寸可读性。
